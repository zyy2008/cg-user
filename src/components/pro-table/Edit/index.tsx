...
// import { ProForm, ProFormText } from '@/components';
...
> = Omit<ProTableProps<RecordType>, "columns" | "request" | "params"> & {
...
const props = {
  ...
  cardBordered: [Boolean, Object] as PropType<
    EditableProTableProps["cardBordered"]
  >,
...
const EditableProTable = defineComponent({
  ...
  setup(props, { slots, emit }) {
    const modelRef = reactive<Record<string, unknown>[]>(
      props.dataSource || []
    );
          ...
          return (
            <Item>
              {column.customRender?.(option) || (
                <Input
                  v-model:value={modelRef[index][column.dataIndex as string]}
                  allowClear
                />
              )}
            </Item>
...
export default EditableProTable as DefineComponent<EditableProTableProps> &
  Plugin;
